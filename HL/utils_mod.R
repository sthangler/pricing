...
plot_act_metrics <- function(data, var, n = 10, categorical = FALSE, exposure, losses, numbers){
  ...
  g <- grid.arrange(
                    # plot_lr, 
                    plot_lc, plot_freq, plot_sev,
                    ncol = 2, nrow = 2
                    # ,
                    # top = textGrob(rlang::as_name(enquo(var)), gp = gpar(fontsize=15))
                    )
                    # top = text_grob(rlang::as_name(enquo(var)), gp = gpar(fontsize=15)))
}
...
actpred <- function(data, var, n = 10, categorical = FALSE, weight, losses, predicted){
  ...
  plot <- table %>%
    ggplot()+
    geom_bar(aes(x = as.factor(feature), y = count * (max(table$exp) / max(table$exp))), alpha = .5,
             stat="identity", fill="peachpuff3")+
    geom_line(aes(x = as.factor(feature), y = act, group = 1), col = scales::hue_pal()(4)[4])+
    geom_line(aes(x = as.factor(feature), y = pred, group = 1), col = scales::hue_pal()(4)[4])+
    xlab(enquo(var))+
    scale_y_continuous(sec.axis = sec_axis(~./((max(table$exp) / max(table$exp))*1000), name = "Numbers (k)"))+
    labs(y = "Severity")+
    ggtitle("Severity")
}
